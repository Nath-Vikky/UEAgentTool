...
- `add_blueprint_node_template`：创建受控 Blueprint 节点模板，例如 `print_string`、`branch_print_string`、`sequence_print_strings`、`delay_print_string`、变量 Get/Set、无参函数调用、Custom Event -> PrintString、Enhanced Input Action 事件节点，以及 `enhanced_input_print_string` 的 `Triggered -> PrintString` 安全链。`print_string` 可连接 `BeginPlay / ActorBeginOverlap / ActorEndOverlap` 这类白名单入口事件。
...
打开插件面板时，UEAgentTool 会自动静默提交一次 Project Inventory；如果后端当时未启动，自动提交会跳过，用户仍可在 Debug View 顶部点击 `Submit Inventory`，或在 Agent Chat / Project QA 的 Function Workspace 点击 `Sync Inventory Now` 手动重试。`Submit Inventory` / `Sync Inventory Now` 会扫描当前工程的 Asset Registry 资产和 `Source/Plugins` 代码摘要，并提交到 `POST /api/v1/project-inventory/snapshot`。这用于让 Project QA 回答项目事实问题，例如工程里有哪些资产、某类资产设置、某个 Blueprint 有哪些组件/变量/函数/图表、某模块有哪些代码文件。插件不会解析 `.uasset` 二进制，也不会修改资产。
...
Agent Chat / Project QA 的 Function Workspace 还提供 `Show Inventory Summary` 和 `Show Blueprint Graphs`。`Show Inventory Summary` 会读取后端只读接口 `GET /api/v1/project-inventory/summary`，在聊天区展示最近一次快照的资产、Blueprint、代码文件、关卡 Actor、材质实例和常见类型计数，方便判断 Agent 当前是否看到了工程上下文。`Show Blueprint Graphs` 如果检测到内容浏览器当前选中了 Blueprint，会优先按选中 Blueprint 查询；否则会读取 `GET /api/v1/project-inventory/blueprint-graphs?include_nodes=true&limit=20` 展示全项目摘要。结果会把最近一次 Project Inventory 中的 Blueprint 图表、节点数、pin 数、连线数和少量节点样本作为系统消息显示在聊天区。这两个按钮只是查看快照，不会创建 Proposal，也不会修改蓝图。
...
- `print_string` can now connect from `BeginPlay`, `ActorBeginOverlap`, or `ActorEndOverlap` when `entry_event` is provided.
