...
void FUEAgentHttpClient::RequestMaterialInstances(const int32 Limit, const FJsonResponseCallback& Callback) const
{
	const int32 SafeLimit = FMath::Clamp(Limit, 1, 500);
	SendRequest(TEXT("GET"), FString::Printf(TEXT("/api/v1/editor-operations/inspect/material-instance-parameters?limit=%d"), SafeLimit), nullptr, Callback);
}
...
void FUEAgentHttpClient::RequestBlueprintGraphs(const FString& BlueprintQuery, const int32 Limit, const bool bIncludeNodes, const FJsonResponseCallback& Callback) const
{
	...
	const FString IncludeNodes = bIncludeNodes ? TEXT("true") : TEXT("false");
	FString RelativePath = FString::Printf(TEXT("/api/v1/project-inventory/blueprint-graphs?include_nodes=%s&limit=%d"), *IncludeNodes, SafeLimit);
	const FString CleanQuery = BlueprintQuery.TrimStartAndEnd();
	if (!CleanQuery.IsEmpty())
	{
		RelativePath += FString::Printf(TEXT("&blueprint_query=%s"), *FGenericPlatformHttp::UrlEncode(CleanQuery));
	}
	SendRequest(TEXT("GET"), RelativePath, nullptr, Callback);
}
...
void FUEAgentHttpClient::CreateEditorOperationFollowUpProposal(
	...
	const FJsonResponseCallback& Callback) const
{
	SendRequest(
		TEXT("POST"),
		FString::Printf(TEXT("/api/v1/editor-operations/proposals/%s/follow-ups/proposal"), *ProposalId),
		RequestObject.IsValid() ? RequestObject : MakeShared<FJsonObject>(),
		Callback);
}
